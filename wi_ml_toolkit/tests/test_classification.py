...
for Model in Classifiers:
    model = Model(np.hstack([X_train, y_train[:, None]]))
    model.fit()
    print('#' * 50, model.__class__.__name__)
    pred = model.predict(X_test)
    print(classification_report(y_test, pred))

model = NeuralNetworkClassifier(np.hstack([X_train, y_train[:, None]]))
model.fit(algorithm='backprop')
print('#' * 50, model.__class__.__name__, '(backprop)')
pred = model.predict(X_test)
print(classification_report(y_test, pred))

model = NeuralNetworkClassifier(np.hstack([X_train, y_train[:, None]]))
model.fit(algorithm='evolution')
